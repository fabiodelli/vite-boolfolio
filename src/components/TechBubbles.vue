...
export default {
  ...
  props: {
    ...
    // il box ha già padding via CSS; non usiamo più un pad “logico” per evitare doppi conteggi
  ...
  data() {
    return {
      boxW: 0,
      boxH: 0,
      bubbles: [], // {name,imageUrl,faIcon,size,mass,x,y,vx,vy,baseSpeed,_el}
      _raf: null,
      _tAccum: 0,
      _lastT: 0,
      _dtFixed: 1 / 120,   // timestep fisso 120Hz
      _dtMax:   1 / 30,    // clamp su frame lenti
      _substepsMax: 5,
    };
  },
  watch: {
  ...
  methods: {
    ...
    measure() {
      ...
      if (!el) return;
      // NB: clientWidth/Height escludono il bordo, includono il padding;
      // poiché le bolle sono assolute “dentro” il padding, l’origine (0,0) è già dopo il padding.
    ...
    onResize() {
      ...
      for (const b of this.bubbles) {
        // clamp nel nuovo box (senza “pad” aggiuntivi)
        b.x = Math.min(Math.max(b.x, 0), Math.max(0, this.boxW - b.size));
        b.y = Math.min(Math.max(b.y, 0), Math.max(0, this.boxH - b.size));
      }
    },
    seedFrom(str = '') {
    ...
    setup() {
      ...
      this.items.forEach(it => {
        ...
        // posizione senza sovrapporsi, nello spazio [0, W-size] x [0, H-size]
        let x, y, attempts = 0;
        do {
          x = r() * Math.max(1, W - size);
          y = r() * Math.max(1, H - size);
          attempts++;
    ...
    start() {
      ...
      const tick = (t) => {
        ...
        this._lastT = t;
        if (dt > this._dtMax) dt = this._dtMax;
        this._tAccum += dt;
    ...
    updatePhysics(dt) {
      ...
      // normalizza velocità: nessun damping → marcia costante (setosa)
      for (const b of this.bubbles) {
        const v = Math.hypot(b.vx, b.vy);
        const minV = b.baseSpeed * 0.98;
        const maxV = b.baseSpeed * 1.02;
        if (v < minV && v > 0) { const s = minV / v; b.vx *= s; b.vy *= s; }
        else if (v > maxV) { const s = maxV / v; b.vx *= s; b.vy *= s; }
      }
    },
  }
};
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- etichetta RIMOSSA come richiesto -->
    </div>
  </div>
</template>
